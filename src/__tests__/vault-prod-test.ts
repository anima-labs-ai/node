...
await test("listCredentials()", async () => {
	...
	if (!r.items.some((i) => i.id === credId))
		throw new Error("created cred not in list");
});

await test("updateCredential(id, input)", async () => {
	const c = await anima.vault.updateCredential(credId, {
		name: "SDK Prod Updated",
	});
	if (c.name !== "SDK Prod Updated")
		throw new Error(`name not updated: ${c.name}`);
});
...
await test("generatePassword()", async () => {
	...
	if (!r.password || r.password.length < 8)
		throw new Error(`bad password: ${r.password}`);
});

await test("generatePassword(opts)", async () => {
	const r = await anima.vault.generatePassword({ length: 32 });
	if (r.password.length !== 32)
		throw new Error(`expected 32 chars, got ${r.password.length}`);
});
